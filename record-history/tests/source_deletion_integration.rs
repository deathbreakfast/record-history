...
async fn cascade_deletes_history_in_both_implementor_tables_happy_path() {
    ...
    let source_delete = E2eHistorySourceA::delete(TEST_SOURCE_A_ID, &valence, valence::use_!(r"**Test:** Fixture **E2e History Source A** remove for `tests` so the suite can arrange and assert persistence behavior. CI and developers running the suite only.")).await;
    match source_delete {
        Ok(()) => {
            let got = E2eHistorySourceA::get(TEST_SOURCE_A_ID, &valence, valence::use_!(r"**Test:** Fixture **E2e History Source A** load for `tests` so the suite can arrange and assert persistence behavior. CI and developers running the suite only.")).await;
            assert!(
                matches!(got, Ok(None))
                    || got
                        .as_ref()
...
async fn delete_empty_source_succeeds_happy_path() {
    ...
    E2eHistorySourceB::delete(helpers::TEST_SOURCE_B_ID, &valence, valence::use_!(r"**Test:** Fixture **E2e History Source B** remove for `tests` so the suite can arrange and assert persistence behavior. CI and developers running the suite only."))
        .await
...
async fn user_delete_history_row_blocked_sad() {
    ...
    let err = E2eRecordHistoryFixture::delete("c4-row", &user_v, valence::use_!(r"**Test:** Fixture **E2e Record History Fixture** remove for `tests` so the suite can arrange and assert persistence behavior. CI and developers running the suite only."))
        .await
    ...
    assert!(
        E2eRecordHistoryFixture::get("c4-row", &valence, valence::use_!(r"**Test:** Fixture **E2e Record History Fixture** load for `tests` so the suite can arrange and assert persistence behavior. CI and developers running the suite only."))
            .await
            .expect("get")
            .is_some(),
