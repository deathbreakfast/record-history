...
async fn e2e_fixture_row_upsert_round_trips_on_sqlite_file_happy_path() {
    ...
    // Non-null actor avoids SQLite null-RecordId readback skew on this pin.
    {
        ...
        .expect("user");
        User::upsert_used("sqlite-fixture-actor", user, &valence, valence::use_!("upsert User in record-history/tests/sqlite_file_fixture_upsert.rs; Valence persistence for this feature path; typed store; visible to test harness."))
            .await
            .expect("seed actor");
    }
    ...
    E2eHistorySourceA::upsert_used(source_rid.id(), source, &valence, valence::use_!("upsert E2eHistorySourceA in record-history/tests/sqlite_file_fixture_upsert.rs; Valence persistence for this feature path; typed store; visible to test harness."))
        .await
    ...
    let upserted = E2eRecordHistoryFixture::upsert_used("e2e-fix-000", row, &valence, valence::use_!("upsert E2eRecordHistoryFixture in record-history/tests/sqlite_file_fixture_upsert.rs; Valence persistence for this feature path; typed store; visible to test harness."))
        .await
    ...
    let loaded = E2eRecordHistoryFixture::get_used("e2e-fix-000", &valence, valence::use_!("get E2eRecordHistoryFixture in record-history/tests/sqlite_file_fixture_upsert.rs; Valence persistence for this feature path; typed store; visible to test harness."))
        .await
        .expect("get")
